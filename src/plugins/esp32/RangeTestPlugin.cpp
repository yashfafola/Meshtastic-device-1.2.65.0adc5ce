...
bool RangeTestPluginRadio::sendPayload(NodeNum dest, bool wantReplies)
{
    ...
    if (!gettimeofday(&tv, NULL)) {
        ...
        int ms = (tv.tv_usec/1000) % (SEC_PER_DAY*1000);
        hms = (hms + SEC_PER_DAY) % SEC_PER_DAY;
        ms = (ms + SEC_PER_DAY*1000) % (SEC_PER_DAY*1000);
        ...
        snprintf(heartbeatString, sizeof(heartbeatString), "seq %u, %02d:%02d:%02d.%.3d", \
            packetSequence, hour, min, sec, ms);
        //fileToAppend.printf("%02d:%02d:%02d,", hour, min, sec); // Time
        //fileToAppend.printf("%.3f,", fms); // Millieconds
    }
    ...
    if (!SPIFFS.exists("/static/txdata.csv")) {
        //--------- Write to file
        File fileToWrite = SPIFFS.open("/static/txdata.csv", FILE_WRITE);
        ...
        if (fileToWrite.println(
                //"Rx Time,from,sender name,sender lat,sender long,rx lat,rx long,rx elevation,rx snr,distance,hop limit,payload")) {
                "Payload Tx")) {    
    ...
    File fileToAppend = SPIFFS.open("/static/txdata.csv", FILE_APPEND);
    ...
    fileToAppend.printf("seq %u \n", packetSequence); // Packet Hop Limit
    fileToAppend.close();
...
bool RangeTestPluginRadio::appendFile(const MeshPacket &mp)
{
    ...
    */
    ...
    if (!SPIFFS.exists("/static/rangetest.csv")) {
        ...
        if (fileToWrite.println(
                //"Rx Time,from,sender name,sender lat,sender long,rx lat,rx long,rx elevation,rx snr,distance,hop limit,payload")) {
                "Rx Time,From,Sender Name,Rx SNR,Hop Limit,Payload Rx")) {    
            DEBUG_MSG("File was written\n");
        } else {
            DEBUG_MSG("File write failed\n");
        }
    ...
    if (!gettimeofday(&tv, NULL)) {
        ...
        // Tear apart hms into h:m:s.ms
        int hour = hms / SEC_PER_HOUR;
        int min = (hms % SEC_PER_HOUR) / SEC_PER_MIN;
        int sec = (hms % SEC_PER_HOUR) % SEC_PER_MIN; // or hms % SEC_PER_MIN

        fileToAppend.printf("%02d:%02d:%02d.%.3d,", hour, min, sec, ms); // Time
        //fileToAppend.printf("%.3f,", fms); // Millieconds
    } else {
